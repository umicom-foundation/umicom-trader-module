...
int main(void)
{
    ...
    assert(umi_trader_runtime_create_panel_view("watchlist", workspace, &view) == UMI_STATUS_OK);
    assert(view != NULL);
    umi_ui_view_model_destroy(view);
    assert(umi_trader_runtime_create_panel_view("scanner", workspace, &view) == UMI_STATUS_NOT_IMPLEMENTED);
